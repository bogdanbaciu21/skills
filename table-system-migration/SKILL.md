...
### 3a. Adding The Canonical Class Is Not Enough — Neutralize Competing Overrides

This is the silent failure of a class-only migration. You add the canonical class, the table validates, the ratchet is green — and it still renders off-brand, because a **pre-existing per-page rule out-specifies the canonical component and wins.**

- **The mechanism.** The canonical header rule is typically `table.<canonical> thead th` (specificity 0,1,3). A leftover page rule like `table.<legacy> thead th { background: #1a2330 }` has the **same** specificity (0,1,3), so the cascade falls through to **source order** — and a page `<style>` block (or any later stylesheet) comes *after* the linked canonical CSS, so the leftover **wins**. The header silently keeps its old color. Multi-class (`.a.b th`), id (`#x th`), and `!important` rules beat the canonical component outright.
- **A `.legacy thead th` (0,1,2) without the `table.` prefix LOSES** to the canonical `table.<c> thead th` (0,1,3) — so it is already overridden and harmless. The dangerous ones are element-prefixed (`table.X …`), multi-class, id-scoped, or `!important`. The bundled ratchet's `canonical_override` check flags exactly these.
- **Remediation: remove the competing paint, do not re-add it.** Delete the `background`/`color` from the offending rule so the canonical styling takes over. **Preserve structural rules** in the same block — `min-width` (horizontal scroll), `position: sticky`, `@media` card-mode, `display:none` hidden columns, numeric `text-align`, special semantic cell classes, `::before/::after` content. Removing the whole block is fine when it is pure cosmetic re-implementation; reduce it to the structural declarations when it is not.
- **Do not blanket-strip — preserve genuine visualizations.** Heatmaps, comparison matrices where header color *encodes data*, frozen-corner pivots, and JS-rendered/sortable tables are deliberate. Allowlist their class and re-baseline; do not flatten them to the canonical look.
- **Verify the canonical style actually wins — by computed style, with the cache bypassed.** Stylesheet caching is notoriously sticky: a page can keep rendering the old header until a hard reload. Confirm the fix by reading the *computed* `background-color` of a `thead th` (e.g. it equals the canonical brand value) after a cache-ignoring reload — never trust source-only or a soft refresh.
...
- Scan content/templates for new unsanctioned table classes, new per-page table CSS, new unwrapped tables, and **`canonical_override`** rules (page-local selectors that out-specify and re-paint the canonical header — see step 3a). The `canonical_override` kind is what catches the "class is present but it still looks wrong" case that the other three checks miss.
...
- Assert that the canonical styling actually wins on a migrated table: read the computed `thead th` `background-color` and confirm it equals the canonical brand value, not a leftover per-page color (the `canonical_override` regression, step 3a). A class-presence assertion is not enough — it passes even when the header is silently overridden.
