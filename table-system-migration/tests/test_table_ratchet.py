...
class TableRatchetTests(unittest.TestCase):
    ...
    def test_flags_canonical_override_but_not_the_canonical_rule_itself(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            page = root / "page.html"
            # Migrated correctly: canonical class + wrapper, and the canonical
            # component's OWN header rule must NOT be flagged as an override.
            clean = (
                "<style>\n"
                "table.content-table thead th { background: #005680; color: #fff; }\n"
                "</style>\n"
                '<div class="table-scroll"><table class="content-table">'
                "<thead><tr><th>H</th></tr></thead><tbody><tr><td>Ok</td></tr></tbody>"
                "</table></div>"
            )
            page.write_text(clean, encoding="utf-8")
            baseline = run_ratchet(
                root, "--canonical-table-class", "content-table",
                "--canonical-wrapper-class", "table-scroll", "--write-baseline",
            )
            self.assertEqual(0, baseline.returncode, baseline.stdout + baseline.stderr)
            self.assertNotIn("canonical_override", (root / ".table-ratchet-baseline.json").read_text(encoding="utf-8"))

            # Now a leftover per-page rule out-specifies the canonical header.
            page.write_text(
                clean.replace(
                    "</style>",
                    "table.legacy thead th { background: #1a2330; color: #fff; }\n</style>",
                ),
                encoding="utf-8",
            )
            regressed = run_ratchet(
                root, "--canonical-table-class", "content-table",
                "--canonical-wrapper-class", "table-scroll",
            )
            self.assertEqual(1, regressed.returncode, regressed.stdout + regressed.stderr)
            self.assertIn("canonical_override", regressed.stdout)
